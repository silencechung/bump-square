...
- `~/.claude/settings.json`：`permissions.additionalDirectories` 加了 `~/Documents/Projects`，並 allow `Edit/Write(/home|/var/home/hotpapa/Documents/Projects/**)`，讓 agent 能在 Projects 下建/改 handoff 出來的元件專案（`/home` 是 `/var/home` 的 symlink，兩種前綴都列）。設定在 session 啟動時載入。
